        ...
        for (const prefix of capabilities) {
          ...
          for (const characteristicMap of [ characteristicMaps ].flat()) {
            ...
            const characteristics = [ characteristicMap.characteristics ].flat().map(klass => {
              ...
              if (map.onUpdate) {
                characteristic.on('change', async ({ oldValue, newValue }) => {
                  ...
                  map.onUpdate({ characteristic, oldValue, newValue, service, device, capability });
                });
              }
              ...
              if (! isTrigger) {
                if (getter) {
                  characteristic.onGet(async () => {
                    ...
                    return await getter(value, { device, service, characteristic : klass });
                  });
                }
                if (setter) {
                  characteristic.onSet(async rawValue => {
                    const value = await setter(rawValue, { device, service, characteristic : klass });
            ...
            device.makeCapabilityInstance(capability, async rawValue => {
              const value = await getter(rawValue, { device, service, capability });
              ...
              if (value !== Mapper.Constants.NO_VALUE) {
