const { Accessory, Service, Characteristic, AccessoryEventTypes } = require('../modules/hap-nodejs');
  ...
  accessorize() {
    ...
    for (const map of this.#maps) {
      ...
      for (const [ group, capabilities ] of Object.entries(groups)) {
        // if this map doesn't allow grouping, re-use existing service if it exists.
        let service = accessory.getService(map.service);
        if (! service || map.group === true) {
          this.log(`creating new service '${ map.service.name }' for group ${ group }`);
          service = accessory.addService(map.service, device.name, group || 'default');
        } else {
          this.log(`using existing service '${ map.service.name }' for group ${ group }`);
        }
        ...
        for (const prefix of capabilities) {
          ...
          // TODO: allow a capability to be both required/optional _and_ a trigger
          ...
          for (const characteristicMap of [ characteristicMaps ].flat()) {
              ...
              if (! isTrigger) {
                if (getter) {
                  characteristic.onGet(async () => {
                    ...
                    return await getter(value, device, service, klass);
                  });
                }
                if (setter) {
                  characteristic.onSet(async rawValue => {
                    const value = await setter(rawValue, device, service, klass);
                    await this.#device.setCapabilityValue(capability, value);
                    // also update `capabilitiesObj` for this device because it doesn't get updated automatically
            ...
            device.makeCapabilityInstance(capability, async rawValue => {
              const value = await getter(rawValue, device, service);
              this.log(`capability update - capability=${ capability } value=${ value.toString() } raw=${ rawValue }`);

              // we only update the characteristics if the getter didn't explicitly return a "no value"
              if (value !== Mapper.NO_VALUE) {
...
Mapper.NO_VALUE                      = Symbol('NO_VALUE');
Mapper.Utils                         = {};
Mapper.Utils.normalizeCapability     = cap => cap.split('.')[0];
Mapper.Utils.normalizeCapabilities   = caps => Object.keys(caps.reduce((acc, cap) => (acc[Mapper.Utils.normalizeCapability(cap)] = true, acc), {}));
Mapper.Utils.hasCapability           = (device, cap) => device.capabilities?.some(capability => Mapper.Utils.normalizeCapability(capability) === cap);
Mapper.Utils.allCapabilitiesMatching = (device, cap) => device.capabilities?.filter(capability => capability === cap || capability.startsWith(`${ cap }.`)) || [];
Mapper.Utils.hasCapabilityWithValue  = (device, cap, value) => Mapper.Utils.allCapabilitiesMatching(device, cap).some(capability => device.capabilitiesObj?.[capability]?.value === value);
Mapper.Utils.upperFirst              = s => String(s).replace(/^./, m => m[0].toUpperCase());
Mapper.Utils.mapValue                = (value, x1, y1, x2, y2) => (value - x1) * (y2 - x2) / (y1 - x1) + x2;
...
Mapper.Accessors = {
  ...
  ColorTemperature : {
    get : async (value, device) => {
      ...
      return Mapper.Utils.mapValue(value, 0, 1, 140, 500);
    },
    set : value => Mapper.Utils.mapValue(value, 140, 500, 0, 1),
  },
  Position : {
    get : value => value * 100,
    set : value => value / 100,
