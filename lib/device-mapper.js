        ...
        for (const prefix of capabilities) {
          ...
          if (! service) {
            ...
            if (! service || map.group === true) {
              this.log(`creating new service '${ map.service.name }' for group '${ group }' (${ capabilities })`);
              service = accessory.addService(map.service, device.name, group || 'default');
            } else {
              this.log(`using existing service '${ map.service.name }' for group '${ group }' (${ capabilities })`);
            }
          }
