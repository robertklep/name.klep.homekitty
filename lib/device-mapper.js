...
const Mapper = module.exports = new (class MapperImpl {
  ...
  // Resolves a device's live RTSP url. Set by the app once the Web API is up;
  // null until then, which disables cameras rather than half-configuring them.
  setVideoResolver(resolver) {
    this.#videoResolver = resolver || null;
  }
  ...
  // Base url of the Homey Web API. The Eufy app publishes a snapshot JPEG per
  // camera there; it is the fallback when the camera's RTSP is not being
  // served, which on Eufy is most of the time.
  setImageBaseUrl(url) {
  ...
  mapDevice(device) {
    ...
    const subMaps = usableMaps
      ...
      .flatMap((subMap) => {
        ...
        if (!map) return [];
        const merged = { ...map, ...subMap.overrides };
        return isPossible(merged) ? [merged] : [];
      });
