...
const getFanSpeed = (device) => {
  return device.capabilitiesObj?.fan_speed ?? device.capabilitiesObj?.dim ?? {};
};
...
module.exports = Mapper => {
  Mapper.Accessors = {
    ...
    RotationSpeed : {
      get: (value, { device }) => {
        const { max = 100 } = getFanSpeed(device);
        return Math.ceil(value * 100 / max);
      },
      set: (value, { device }) => {
        const { min = 0, max = 100, step = 1 } = getFanSpeed(device);
