...
async function getStreamUrl(api, device) {
  ...
  // The owning app can be restarting, updating, or gone. Asking it for a url
  // then fails, and a camera with no url is an ordinary situation -- it is what
  // the snapshot layers exist for -- not a reason to take the bridge down with
  // an unhandled rejection.
  const result = await api.videos.getVideoUrl({ id : video.videoObj.id }).catch(() => null);
