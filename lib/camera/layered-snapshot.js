...
class LayeredSnapshotSource {
  ...
  constructor({ sources, ttlMs = 4000, deadlineMs = 4000, cooldownMs = 60000, log = () => {}, now = Date.now }) {
  ...
  async #fetch() {
    ...
    for (const source of this.#sources) {
      // A source that just failed is unlikely to work seconds later. Eufy's
      // RTSP is down far more often than up, and retrying it on every poll
      // spawns an ffmpeg process per camera every few seconds -- enough load
      // on a Homey to make requests miss HomeKit's window, which looks like
      // pictures randomly coming and going.
      const skipUntil = this.#skipUntil.get(source.name) || 0;
      if (skipUntil > this.#now()) {
        failures.push(`${ source.name }: cooling down`);
        continue;
      }
      ...
      try {
        ...
        this.#skipUntil.set(source.name, this.#now() + this.#cooldownMs);
