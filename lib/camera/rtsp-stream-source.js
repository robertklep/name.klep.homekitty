...
// The address we advertise back to HomeKit must be the one it can reach us on.
function localAddress(family = 'IPv4') {
  for (const addresses of Object.values(os.networkInterfaces())) {
    for (const address of addresses || []) {
      if (address.family === family && ! address.internal) return address.address;
    }
  }
...
class RtspStreamSource {
  ...
  async prepare(request, callback) {
    try {
      ...
      const response = {
        address : localAddress(request.addressVersion === 'ipv6' ? 'IPv6' : 'IPv4'),
        video   : {
