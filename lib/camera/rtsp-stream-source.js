  ...
  async handle(request, callback) {
    ...
    try {
      ...
      const args = [
        ...
        // H.264 parameter sets (SPS/PPS) appear once at the start of the RTSP
        // stream. HomeKit joins partway through and does not take them from
        // SDP, so without repeating them in-band it receives a perfectly valid
        // packet flow it cannot decode -- packets arrive, nothing renders.
        '-bsf:v', 'dump_extra',
