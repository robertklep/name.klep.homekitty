...
const diag = require('./_diag'); // TEMPORARY
...
// Builds the ffmpeg invocation for a stream.
//
// Split out so it can be tested without spawning anything, because the bug it
// exists to prevent is invisible otherwise: the destination port and the SRTP
// keys are settled during prepare and live on the session. A start request
// carries only the negotiated encoding. A test harness that builds its start
// request by copying its prepare request supplies fields HomeKit never sends,
// and every assertion passes while the real thing sends to port `undefined`.
function streamArgs({ url, session, request }) {
  const { port, srtp_key, srtp_salt, ssrc } = session.video;
  const target = `srtp://${ session.targetAddress }:${ port }` +
                 `?rtcpport=${ port }&pkt_size=1316`;
  ...
  return [
    ...
    '-srtp_out_params', srtpParams({ srtp_key, srtp_salt }),
    target,
  ];
}
...
class RtspStreamSource {
  ...
  async handle(request, callback) {
    ...
    try {
      const [ url, ffmpeg ] = await Promise.all([ this.#getUrl(), resolveFfmpeg(m => { diag.record(`ffmpeg: ${ m }`); this.#log(m); }) ]);
      diag.record(`resolved url+ffmpeg (url ${ url ? 'ok' : 'MISSING' })`); // TEMPORARY
      if (! url) throw Error('camera has no RTSP stream');

      diag.record(`START ${ request.video.width }x${ request.video.height }@${ request.video.fps } ${ request.video.max_bit_rate }k -> ${ session.targetAddress }:${ session.video.port }`); // TEMPORARY
      const args = streamArgs({ url, session, request });
...
module.exports = { RtspStreamSource, localAddress, srtpParams, streamArgs };
