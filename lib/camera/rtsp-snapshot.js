...
class RtspSnapshotSource {
  ...
  async get() {
    ...
    try {
      ...
      // Eufy serves a camera's RTSP only intermittently, so a fetch failing is
      // normal rather than exceptional. HomeKit polls every few seconds; the
      // last real frame is far better than a placeholder, and matches what a
      // camera tile is meant to show.
      if (this.#cached) {
        this.#log(`stream unavailable, serving last frame (${ error.message })`);
        return this.#cached;
      }
      throw error;
    } finally {
