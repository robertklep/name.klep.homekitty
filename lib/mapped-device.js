          ...
          for (const characteristicMap of [ characteristicMaps ].flat()) {
            ...
            const characteristics = [ characteristicMap.characteristics ].flat().map(klass => {
              ...
              if (! isTrigger) {
                ...
                if (setter) {
                  // a rejected write (setCapabilityValue throwing) fails the
                  // set request: HAP reports the error to the controller and
                  // leaves the characteristic untouched, and the internal
                  // device state is only updated when the write succeeds
