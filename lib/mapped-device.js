...
module.exports.MappedDevice = class MappedDevice {
  ...
  attachCamera(accessory) {
    ...
    // Picture sources, most live first. A camera needs at least one: HomeKit
    // asks for a snapshot every few seconds and an accessory that can never
    // answer is worse than not being a camera at all.
    const sources = [];

    if (resolver && hasVideo(device)) {
      const getUrl = () => resolver(device);
      sources.push({ name : 'rtsp', get : () => new RtspSnapshotSource({ getUrl, log }).get() });
    }

    const image = baseUrl && findSnapshotImage(device);
    if (image) {
    ...
    if (! sources.length) {
      this.log(2, '- camera skipped: no live stream and no published image');
      return;
    }

    // Live video only exists where there is an RTSP stream. Everywhere else the
    // accessory is a stills camera, which HomeKit is perfectly happy with --
    // it renders the snapshot and only fails if you tap through to live view.
    const streamSource = (resolver && hasVideo(device))
      ? new RtspStreamSource({ getUrl : () => resolver(device), log })
      : undefined;
    ...
    try {
      ...
      this.log(2, `- camera configured (${ sources.map(s => s.name).join(' + ') })`);
    } catch (e) {
