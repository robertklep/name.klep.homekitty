...
module.exports.MappedDevice = class MappedDevice {
  ...
  groupCapabilities() {
    return this.#device.capabilities.reduce((acc, cap) => {
      ...
      // XXX: someone thought it would be a great idea to add
      // a `button.remove_node` capability to *all* Matter devices. Since this
      // actually removes the device from Homey, we'll make sure not to map
      // those capabilities to anything.
      if (capability === 'button' && group === 'remove_node') return acc;
