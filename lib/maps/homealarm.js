module.exports = (Mapper, Service, Characteristic) => {
  const { STAY_ARM, AWAY_ARM, DISARMED, ALARM_TRIGGERED } = Characteristic.SecuritySystemCurrentState;
  const { DISARM }                                        = Characteristic.SecuritySystemTargetState;
  const { TAMPERED }                                      = Characteristic.StatusTampered;

  // keep track of target state
  ...
  const characteristic = {
    ...
    get : (value, { device, service, characteristic }) => {
      ...
      switch(value) {
        ...
        case 'partially_armed': return STAY_ARM;
    ...
    set : (value, { characteristic }) => {
      ...
      switch(value) {
        ...
        case STAY_ARM:  return 'partially_armed';
  ...
  const isTriggerState = state => [ STAY_ARM, AWAY_ARM ].includes(state);
